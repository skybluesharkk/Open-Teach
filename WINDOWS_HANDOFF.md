...
### 네트워크 전환 (중요)
- 윈도우 PC를 **같은 WiFi(ASUS_38, 192.168.50.x)**에 연결
- `ipconfig`로 윈도우 IP 확인 (예: 192.168.50.xxx)
- 퍼블리셔 실행: `python tactile_viz_dummy.py --host <윈도우IP>`
- **Quest 앱 안에서 IP를 <윈도우IP>로 재입력** (기존 49는 리눅스 PC)
- 방화벽이 물으면 Python 허용 (또는 15002/tcp 인바운드 허용)
